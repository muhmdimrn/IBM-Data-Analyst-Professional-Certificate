...
-- Execute a working query using a sub-select to retrieve all employees records whose salary 
...
-- Execute a failing query (i.e. one which gives an error) to retrieve all employees records with EMP_ID, 
...
-- Execute a Column Expression that retrieves all employees records with EMP_ID, SALARY and maximum 
